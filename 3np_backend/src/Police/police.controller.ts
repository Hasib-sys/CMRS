...
UpdatePoliceM(@Session() session,@Body() update_police: PRegistrationDTO , @UploadedFile()myfileobj: Express.Multer.File ): any{
    update_police.profile_image = myfileobj.filename;
  return this.policeService.UpdatePoliceM(session.username, update_police);
...
@Get('/wantedlist/')
@UseGuards(SessionGuard)
SearchWantedList(@Session() session):  any{
    return this.policeService.SearchWantedList(session.username);
}
...
DeleteWantedlist1(@Param('c_no') c_no:number ): object{
    ...
    @UseGuards(SessionGuard)
    //@UsePipes(new ValidationPipe())
    DeletePoliceProfileM(@Session() session): Promise<PRegistrationDTO>{
        session.destroy();
        return this.policeService.DeletePoliceProfileM(session.username);
    }
...
@Get('/scrimestatus/')
@UseGuards(SessionGuard)
SearchCrimeStatus(@Session() session): any{
    return this.policeService.SearchCrimeStatus(session.username );
}
